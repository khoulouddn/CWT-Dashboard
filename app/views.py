...
def index(request): 
   if request.method == "POST":
       ...
       if Profile.objects.filter(compteid=compteid, apassword=apassword , role=a ).exists():
          return redirect('homea')

       if Profile.objects.filter(compteid=compteid, apassword=apassword , role=u ).exists():
          return redirect('homeu')

       if Profile.objects.filter(compteid=compteid, apassword=apassword , role=s ).exists():
...
def dashboarda(request):
    all_datas = Data.objects.all()
    return render(request,'dashboarda.html', {'Datas': all_datas})

def dashboardu(request):
    all_datas = Data.objects.all()
    return render(request,'dashboardu.html', {'Datas': all_datas})
